...
public class PlayerBGMController : MonoBehaviour
{
    ...
    private SoundManager3.EBgm currentBGM; // 현재 재생 중인 BGM 추적
    ...
    private void Update()
    {
        if (playerTransform == null || SoundManager3.instance == null)
        {
        ...
        SoundManager3.EBgm targetBGM;

        // Y축 위치에 따라 BGM 선택
        if (yPosition <= 180f)
        {
            targetBGM = SoundManager3.EBgm.Bgm_StageBattle; // BGM 1번
        }
        else if (yPosition > 180f && yPosition <= 220f)
        {
            targetBGM = SoundManager3.EBgm.Bgm_City; // BGM 2번 (가정)
        }
        else if (yPosition > 220f && yPosition <= 330f)
        {
            targetBGM = SoundManager3.EBgm.Bgm_Enterprise; // BGM 3번 (가정)
        }
        else if (yPosition >= 330f)
        {
            targetBGM = SoundManager3.EBgm.Bgm_BossBattle; // BGM 3번 (가정)
        }
        else
        {
            targetBGM = SoundManager3.EBgm.Bgm_Ending;
        }
        ...
        if (currentBGM != targetBGM)
        {
            ...
            SoundManager3.instance.PlayBGM(currentBGM);
