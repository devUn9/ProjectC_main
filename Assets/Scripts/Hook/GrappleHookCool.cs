...
public class GrappleHookCool : MonoBehaviour
{
    // 로프 시각화를 위한 LineRenderer 컴포넌트
    private LineRenderer line;

    [Header("Grappling 설정")]
    [SerializeField] private LayerMask grapplableMask;   // 그래플링 가능한 오브젝트의 레이어 마스크
    [SerializeField] private float maxDistance = 10f;     // 그래플링 최대 거리
    [SerializeField] private float grappleSpeed = 10f;    // 끌려오는 속도 (플레이어나 오브젝트)
    [SerializeField] private float grappleShootSpeed = 20f; // 훅 발사 속도 (현재 미사용)
    [SerializeField] private float grappleCooldown = 2f;   // 그래플링 쿨타임 (초)

    private bool isGrappling = false;             // 그래플링 중인지 여부
    private bool isOnCooldown = false;            // 쿨타임 중인지 여부
    ...
    private void Update()
    {
        ...
        if (!isGrappling && !isOnCooldown && Input.GetMouseButton(1))
        {
            LockTarget(); // 지속적으로 Raycast로 감지 (대상 있을 때만 조준점 표시)
        }

        // 우클릭 뗄 때: 조준이 완료된 상태면 실행
        if (!isGrappling && !isOnCooldown && Input.GetMouseButtonUp(1) && isTargetLocked)
        {
